...
|---|---|---|---|
| MG-00 Discovery and decision baseline | Module 1 baseline, Module 2 discovery, Module 3 decision | `docs/01.01-current-state-assessment.md`, `docs/02.01-discovery-findings-summary.md`, `docs/03.01-migration-pattern-assessment.md` | Migration architect |
| MG-01 Build and container readiness | Java runtime, Maven dependencies, container image, build egress | `inventory/02.01-app-inventory.json`, `inventory/02.01-app-inventory.csv`, egress F004/F005 | Application lead |
| MG-02 Database readiness | H2 local profile, PostgreSQL, MySQL | `inventory/02.04-database-inventory.csv`, egress F002/F003 | Database owner |
| MG-03 Managed container replatform | ACR, Azure Container Apps, app configuration, health checks | `inventory/03.01-migration-decision-matrix.csv`, ADR-0002 | Platform owner |
| MG-04 Network and security hardening | Ingress, DNS, TLS, SMTP-like egress, unknown inbound, secrets | `inventory/09.01-ingress-inventory.csv`, `inventory/02.03-egress-inventory.csv`, `docs/03.04-assumptions-risk-register.md` | Network/security owner |
| MG-05 Cutover and hypercare | Release orchestration, smoke tests, rollback, monitoring | `docs/11.02-cutover-runbook.md`, `docs/11.03-rollback-plan.md`, `tests/smoke_test.sh`, `docs/11.04-hypercare-checklist.md` | Release manager |
...
|---|---|
| PostgreSQL/MySQL DB flows exist in `02.03-egress-inventory.csv` | Database readiness must happen before app cutover |
| Unknown inbound TCP `8443` exists in `09.01-ingress-inventory.csv` | Network validation must happen before DNS/TLS cutover |
| SMTP-like TCP `25` egress exists in `02.03-egress-inventory.csv` | App owner must confirm whether notifications/integration exist |
...
- [`inventory/03.03-wave-plan.csv`](../inventory/03.03-wave-plan.csv)
...
| Rollback owner assigned | `docs/11.03-rollback-plan.md` and owner approval |
