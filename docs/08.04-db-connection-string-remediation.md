...
The application supports environment-driven PostgreSQL configuration through `src/main/resources/application-postgres.properties`. The password property has no committed fallback value; it must come from the runtime environment, Key Vault or the platform secret store.
...
| Evidence | File |
|---|---|
| Runtime environment matrix | `inventory/05.01-runtime-environment-matrix.csv` |
...
| Live cutover evidence | `evidence/logs/08.29-db-post-cutover-smoke-test-results.md` |
