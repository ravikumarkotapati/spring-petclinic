...
| Evidence | `inventory/09.01-ingress-inventory.csv`, `infra/container-apps/petclinic-containerapp-ingress-networking.yaml`, `k8s/ingress.yaml` |
...
The detailed allowlist is in `inventory/09.02-network-egress-allowlist.csv`.
...
The before-and-after dependency map is captured in `docs/09.04-network-dependency-before-after.mmd`.
...
|---|---|---|
| Define external ingress | Complete | `inventory/09.01-ingress-inventory.csv`, `infra/container-apps/petclinic-containerapp-ingress-networking.yaml`, `k8s/ingress.yaml` |
| Define internal ingress | Complete | Internal ingress section in this document and `inventory/09.01-ingress-inventory.csv` |
| Define egress for DB, Key Vault, third-party API, SMTP, file share and auth provider | Complete | `inventory/02.03-egress-inventory.csv`, `inventory/09.02-network-egress-allowlist.csv` |
| Create egress allowlist | Complete | `inventory/09.02-network-egress-allowlist.csv` |
| Explain enforcement with NSGs, Azure Firewall, NAT Gateway, Private Endpoints and NetworkPolicies | Complete | Enforcement design section and `docs/09.02-firewall-egress-design.md` |
| Show before-and-after network dependency map | Complete | `docs/09.04-network-dependency-before-after.mmd` |
| Provide DNS/TLS cutover plan | Complete | `docs/09.03-dns-tls-cutover-plan.md` |
